...
(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            state <= IDLE;
        end else if (ctrl_negedge) begin
            state <= IDLE;  // 控制信号下降沿立即重置
        end else begin
            state <= next_state;
        end
    end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if ((state == FILLING || state == REFILLING) && write_addr == BUFFER_DEPTH - 1 && input_handshake) begin
                ...
                buffer_full <= 1'b0;
            end else if (state == REFILLING && write_addr == {ADDR_WIDTH{1'b0}}) begin
                buffer_full <= 1'b0;  // 进入REFILLING时立即清除满标志
            end
    ...
    always @(posedge clk) begin
        ...
        if (ctrl_posedge) begin
            $display("[%0t] Data_Buffer: Control POSEDGE - Start Sending", $time);
        end
